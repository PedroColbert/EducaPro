...
interface GuardianLoginForm {
    email: string;
    password: string;
    remember: boolean;
}

export default function GuardianLogin({ status }: { status?: string | null }) {
    const [showPassword, setShowPassword] = useState(false);
    const { data, setData, post, processing, errors, reset } = useForm<GuardianLoginForm>({
        email: '',
        password: '',
        remember: false,
    });
    ...
    return (
        <AuthLayout title="Acesso da familia" description="Entre com o login do responsavel para acompanhar presenca, agenda, atividades e recados importantes.">
            ...
            <div className="mb-6 rounded-2xl border border-indigo-100 bg-indigo-50 px-4 py-3 text-sm text-indigo-700">
                <div className="flex items-center gap-2 font-medium">
                    <Users size={16} />
                    Acesso pensado para leitura rapida, acompanhamento de mais de um filho e uso simples no celular.
                </div>
            </div>
            ...
            <form onSubmit={submit} className="space-y-6" noValidate>
                ...
                <div className="space-y-2">
                    <label className="block text-sm font-semibold text-slate-700" htmlFor="guardian-email">
                ...
                <div className="space-y-2">
                    <label className="block text-sm font-semibold text-slate-700" htmlFor="guardian-password">
                        Senha
                    </label>
                    <div className="relative">
                ...
                <div className="flex items-center">
                    <input id="guardian-remember" type="checkbox" checked={data.remember} onChange={(event) => setData('remember', event.target.checked)} className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
                    <label htmlFor="guardian-remember" className="ml-2 text-sm text-slate-600">
                ...
                <button type="submit" disabled={processing} className={cn('group flex w-full items-center justify-center gap-2 rounded-xl py-3.5 font-medium text-white shadow-sm shadow-indigo-200 transition-colors', processing ? 'cursor-wait bg-indigo-500' : 'bg-indigo-600 hover:bg-indigo-700')}>
                    <span>{processing ? 'Entrando...' : 'Entrar no portal da familia'}</span>
